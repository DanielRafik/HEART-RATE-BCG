...
all_data=[ecg1,bcg1]
fig=plt.figure()
plt.title("Box Plot of ECG heart rates and BCG heart rates of patient no:2")
plt.boxplot(all_data)
...
df=pd.DataFrame({'ECG':ecg,
                'BCG':bcg})
sm.graphics.mean_diff_plot(df["ECG"],df["BCG"])
plt.title("Bland-Altman Plot of ECG heart rates and BCG heart rates of patient no:2")
...
var1 = pd.Series (ecg)  
var2 = pd.Series (bcg)  
plt.title ('Correlation between ECG and BCG of patient 40')  
...
var1 = pd.Series (ecg1)  
var2 = pd.Series (bcg1)  
plt.title ('Correlation between ECG and BCG of patient 1')  
plt.scatter (var1, var2)  
plt.plot(np. unique (var1), np.poly1d (np.polyfit(var1, var2, 1))(np.unique (var1)), color = 'green')
